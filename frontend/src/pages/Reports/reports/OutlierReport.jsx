...
const OutlierReport = ({ highlightedOutlierId }) => {
    ...
    useEffect(() => { const fetchOutliers = async () => { setIsLoadingOutliers(true); setOutlierError(null); try { const filters = { startDate: dateRange[0], endDate: dateRange[1] }; const data = await ReportsService.getOutlierReports(filters); setOutlierData(data); console.log("Outlier Reports Fetched:", data); } catch (err) { console.error("Failed to fetch outliers:", err); setOutlierError(err.detail || "Could not load outliers."); setOutlierData([]); } finally { setIsLoadingOutliers(false); } }; fetchOutliers(); }, [dateRange]);
    ...
    const handleExportCSV = () => {
        const headers = ['Date', 'Driver', 'Vehicle', 'FleetEdge (km/l)', 'Bill (km/l)', 'Variance'];
        const csvContent = [
            headers.join(','),
            ...filteredRows.map(row => [
                ...
                typeof row.fleetedge_mileage_kml === 'number' ? row.fleetedge_mileage_kml.toFixed(2) : '-',
                typeof row.bill_mileage_kml === 'number' ? row.bill_mileage_kml.toFixed(2) : '-',
        ...
        link.download = `outlier_report_${dayjs().format('YYYY-MM-DD')}.csv`;
        link.click();
    };
    ...
    const handleExportExcel = () => {
        ...
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = `outlier_report_${dayjs().format('YYYY-MM-DD')}.xlsx`;
        link.click();
    };
