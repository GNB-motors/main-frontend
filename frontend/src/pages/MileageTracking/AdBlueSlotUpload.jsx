...
/** A single drag-and-drop / browse / preview slot for one receipt image. */
export const SlotUpload = (props) => {
  const { title, label, inputId, required, doc, isScanning, onDrop, onRemove } = props;
  ...
  return (
    <div className="modern-document-slot">
      <div className="slot-header">
        ...
        {!required && (
          <span style={{ fontSize: 12, color: '#94a3b8', fontWeight: 500 }}>Optional</span>
        )}
      </div>
      <input
        type="file"
            ...
            <div className="slot-image-overlay">
              <div className={`slot-badge ${doc.ocrStatus === 'success' ? 'success' : ''}`}>
                {isScanning ? (
                  ...
                  <>
                    <CheckCircle size={13} className="badge-icon-svg" /> Uploaded
                  </>
                ) : (
                  <>
                    <Loader2 size={13} className="spinning" /> Scanning...
