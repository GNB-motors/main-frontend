    ...
    <div className="bem-sidepanel-overlay" onClick={onClose}>
      <div className="bem-sidepanel-container" onClick={(e) => e.stopPropagation()}>
                      ...
                      {uploadResult.created.map((created) => {
                        ...
                        return (
                          <tr key={created.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                            ...
                            <td
                              style={{ padding: '12px', fontFamily: 'monospace', color: '#111827' }}
                            >
                              {password}
                            </td>
                          </tr>
              ...
              <div>
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const csvHeader = 'Row,Code,Error,Field\n';
                      const csvContent = uploadResult.errors
                        .map((err) => {
                          const row =
                            err.index !== undefined && err.index !== null ? err.index + 1 : '-';
                          const code = err.code || 'ERROR';
                          const msg = (err.error || '').replace(/"/g, '""');
                          const field = err.field || '';
                          return `"${row}","${code}","${msg}","${field}"`;
                        })
                        .join('\n');
                      ...
                      link.href = url;
                      link.setAttribute('download', 'employee_upload_errors.csv');
                ...
                <div style={{ maxHeight: '300px', overflowY: 'auto' }}>
                  {uploadResult.errors.map((error, idx) => (
                    ...
                    >
                      ...
                      >
                        Row{' '}
                        {error.index !== undefined && error.index !== null ? error.index + 1 : '-'}:{' '}
                        {error.code || 'ERROR'}
                      </div>
        ...
        <div className="bem-sidepanel-actions">
          <NewButton variant="primary" size="md" type="button" text="Done" onClick={onClose} />
        </div>
      </div>
    </div>
  );
