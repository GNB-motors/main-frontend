...
export const DriverService = {
  ...
  addBulkDrivers: async (employeesArray) => {
    try {
      ...
      if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
        throw {
          detail:
            'Request timed out. The upload may still be processing. Please check the employees list.',
        };
      }
  ...
  updateDriver: async (businessRefId, driverId, driverData) => {
    try {
      ...
      if (driverData.mobileNumber !== undefined)
        body.mobileNumber = normaliseMobile(driverData.mobileNumber);
  ...
  deleteDriver: async (businessRefId, driverId) => {
    try {
      const response = await apiClient.delete(`/api/employees/${driverId}`, {
        params: { orgId: businessRefId },
      });
