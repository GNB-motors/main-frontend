...
const rateOf = (v) => (v == null || Number.isNaN(Number(v)) ? '—' : `₹${formatNum(v, { decimals: 2 })}`);
const dayTick = (d) => {
  const dt = new Date(d);
...
function DailyTooltip({ active, payload }) {
  ...
  return (
    <div className="cluster-inset flex flex-col gap-1 px-3 py-2 text-xs">
      ...
      <span className="num" style={{ color: 'var(--gnb-400)' }}>{formatINR(d.amountInr)}</span>
      <span className="num" style={{ color: 'var(--caution)' }}>{formatLitres(d.litres)}</span>
...
export default function FuelSpendPage() {
  ...
  const { data: summary, loading: summaryLoading, error: summaryError } = useApi(
    (signal) => FleetDataService.getFuelSpendSummary(windowParams, signal),
    [preset],
  );

  const { data: recordsData, loading: recordsLoading, error: recordsError } = useApi(
    (signal) => FleetDataService.getFuelSpendRecords({ ...windowParams, page, limit: PAGE_SIZE }, signal),
    [preset, page],
  );
  ...
  const exportCsv = () => {
    const csv = buildCsvString(
      ['Refuel time', 'Vehicle', 'Fuel type', 'Filling type', 'Litres', 'Rate (INR/L)', 'Amount (INR)', 'Location', 'Mileage (km/L)'],
      records.map((r) => [
    ...
    triggerFileDownload(csv, `fuel-spend-${preset}-page-${page}.csv`, 'text/csv');
  };
    ...
    <div className="cluster-page space-y-5">
        ...
        <div className="flex items-center gap-2">
          {WINDOWS.map((w) => (
            <button
              ...
              style={preset === w.key ? { borderColor: 'var(--gnb-400)', color: 'var(--gnb-400)' } : { color: 'var(--cluster-text-dim)' }}
            >
              {w.days ? w.key : 'All'}
            </button>
          ))}
      ...
      <PanelErrorBoundary name="fuel-spend">
        {summaryLoading && !summary ? (
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <StatTile label="Total spend" value={formatInrCompact(totals.amountInr)} tone="var(--cluster-text)" />
            <StatTile label="Litres" value={formatLitres(totals.litres)} tone="var(--cluster-text)" />
            <StatTile label="Fill-ups" value={formatNum(totals.logCount)} tone="var(--cluster-text)" />
            <StatTile
              label="Avg rate"
        ...
        <div className="cluster-panel mt-4 overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-2 px-4 pt-4">
            ...
            <span className="flex items-center gap-3 text-[11px]" style={{ color: 'var(--cluster-text-dim)' }}>
              <span className="flex items-center gap-1.5">
                <span className="inline-block h-2 w-2 rounded-full" style={{ background: 'var(--gnb-400)' }} /> Spend (₹)
              </span>
              <span className="flex items-center gap-1.5">
                <span className="inline-block h-2 w-2 rounded-full" style={{ background: 'var(--caution)' }} /> Litres
              </span>
            </span>
          </div>
          {summaryLoading && !summary ? (
            <div className="p-4"><div className="cluster-inset h-[280px] animate-pulse" /></div>
          ) : summaryGone ? (
            <div className="p-4">
              <EmptyState
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={daily} margin={{ top: 12, right: 8, left: 0, bottom: 4 }}>
                  ...
                  <YAxis
                    ...
                    width={44}
                  />
                  <Tooltip content={<DailyTooltip />} cursor={{ fill: 'var(--hairline)', opacity: 0.3 }} />
                  <Bar yAxisId="left" dataKey="amountInr" name="Spend" fill="var(--gnb-400)" radius={[4, 4, 0, 0]} maxBarSize={36} />
                  <Line yAxisId="right" type="monotone" dataKey="litres" name="Litres" stroke="var(--caution)" strokeWidth={2} dot={false} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          )}
                  ...
                  <table className="w-full min-w-[560px] text-sm">
                    ...
                    <tbody>
                      {byLocation.map((loc, i) => {
                        ...
                        const swings = loc.maxRateInrPerL != null && loc.minRateInrPerL != null && spread > PRICE_SWING_THRESHOLD;
                        return (
                          <tr key={loc.location || i} style={{ borderBottom: '1px solid var(--hairline)' }}>
                            <td className="px-4 py-3">
                              <span>{loc.location || '—'}</span>
                              {swings ? <span className="lamp lamp--caution ml-2">price swings</span> : null}
                            </td>
                            <td className="num px-4 py-3 text-right">{formatNum(loc.logCount)}</td>
                            <td className="num px-4 py-3 text-right">{formatNum(loc.litres, { decimals: 1 })}</td>
                            <td className="num px-4 py-3 text-right font-semibold">{rateOf(loc.avgRateInrPerL)}</td>
                            <td className="num px-4 py-3 text-right">{rateOf(loc.minRateInrPerL)}</td>
                            <td className="num px-4 py-3 text-right">{rateOf(loc.maxRateInrPerL)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                  ...
                  <tbody>
                    {records.map((r) => (
                      <tr key={r.id} style={{ borderBottom: '1px solid var(--hairline)' }}>
                        ...
                        <td className="num px-4 py-3 text-right font-semibold">{formatINR(r.totalAmount)}</td>
                        <td className="px-4 py-3">
                          <span className="block max-w-[260px] truncate" title={r.location || ''}>{r.location || '—'}</span>
                        </td>
                        <td className="num px-4 py-3 text-right">{r.mileageKmPerL != null ? formatNum(r.mileageKmPerL, { decimals: 1 }) : '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
              <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-3" style={{ borderTop: '1px solid var(--hairline)' }}>
                <span className="num text-dim text-xs">
                  Page {formatNum(page)} of {formatNum(totalPages)} · {formatNum(recordsData?.total)} receipts
                </span>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
