...
function makeCtx() {
  ...
  Object.defineProperty(ctx, 'shadowBlur', {
    ...
    set: (v) => {
      _shadowBlur = v;
      calls.push({ m: 'set', prop: 'shadowBlur', args: [v] });
    },
  });
  const grad = (m, args) => {
    const g = { stops: [] };
...
describe('draw — links', () => {
  it('colours each link as a gradient between the endpoint kind hues', () => {
    ...
    const stops = ctx.calls
      .filter((c) => c.m === 'addColorStop' && c.args[1].startsWith('rgba'))
      .map((c) => c.args);
    expect(stops[0]).toEqual([0, hexa(kindHue('module', 'dark'), 0.14)]);
    expect(stops[1]).toEqual([1, hexa(kindHue('store', 'dark'), 0.14)]);
    const linkStroke = ctx.calls.find(
      (c) => c.m === 'stroke' && c.args.length === 0 && c.lineWidth === 0.7,
    );
...
describe('draw — particles', () => {
  ...
  it('draws particles only on links with traffic, coloured by the target hue', () => {
    ...
    const fills = ctx.calls.filter(
      (c) => c.m === 'fill' && c.fillStyle === hexa(kindHue('store', 'dark'), 0.5),
    );
  ...
  it('highlighted links get radius 1.9 at alpha 0.95 and dimmed links get none', () => {
    ...
    draw(
      ...
      trafficModel({
        ...
        links: [
          { s: 'a', t: 'b', w: 0.8, traffic: true },
          { s: 'c', t: 'd', w: 0.8, traffic: true },
        ],
      }),
    );
    ...
    expect(
      ctx.calls.filter(
        (c) => c.m === 'fill' && c.fillStyle === hexa(kindHue('store', 'dark'), 0.95),
      ).length,
    ).toBe(3);
    expect(
      ctx.calls.filter((c) => c.m === 'fill' && c.fillStyle === hexa(kindHue('job', 'dark'), 0.5))
        .length,
    ).toBe(0);
  });
});
...
describe('draw — the three node treatments', () => {
  it('a declared node NEVER fills with its kind hue', () => {
    ...
    const hueFills = ctx.calls.filter(
      (c) =>
        c.m === 'fill' &&
        (c.fillStyle === hue || String(c.fillStyle).startsWith(hexa(hue, hexa(hue, 0).slice(-4)))),
    );
    ...
    expect(
      ctx.calls.some((c) => c.m === 'setLineDash' && c.args[0][0] === Math.max(2, 4.6 * 0.5)),
    ).toBe(true);
    expect(ctx.calls.some((c) => c.m === 'stroke' && c.strokeStyle === CANVAS.dark.innerRim)).toBe(
      true,
    );
    // radius floored at 4.6
    const voidFillIdx = ctx.calls.findIndex(
      (c) => c.m === 'fill' && c.fillStyle === CANVAS.dark.void,
    );
  ...
  it('an unreachable node emits exactly one moveTo/lineTo pair — the diagonal slash', () => {
    ...
    const slashes = ctx.calls.filter(
      (c) =>
        (c.m === 'moveTo' && c.args[0] === 95 && c.args[1] === 105) ||
    ...
    expect(
      ctx.calls.some(
        (c) =>
          c.m === 'stroke' &&
          c.strokeStyle === hexa(CANVAS.dark.faultCss, 0.95) &&
          c.lineWidth === 2,
      ),
    ).toBe(true);
    expect(ctx.calls.some((c) => c.m === 'setLineDash' && c.args[0].join() === '2,3')).toBe(true);
    expect(ctx.calls.some((c) => c.m === 'fill' && c.fillStyle === CANVAS.dark.voidFault)).toBe(
      true,
    );
  });
  ...
  it('a measured node in dark theme glows at min(22, r*1.7) in hexa(hue, .75)', () => {
    ...
    expect(
      ctx.calls.some((c) => c.m === 'set' && c.prop === 'shadowBlur' && c.args[0] === 17),
    ).toBe(true);
  ...
  it('an error-bearing selected node draws BOTH the pip and the selection ring', () => {
    ...
    const selRing = ctx.calls.find(
      (c) => c.m === 'stroke' && c.strokeStyle === CANVAS.dark.selRing && c.lineWidth === 1.6,
    );
    expect(selRing).toBeTruthy();
    expect(
      ctx.calls.some(
        (c) => c.m === 'arc' && c.args[2] === 15 && c.strokeStyle === CANVAS.dark.selRing,
      ),
    ).toBe(true);
    expect(
      ctx.calls.some(
        (c) => c.m === 'arc' && c.args[2] === 19.5 && c.strokeStyle === CANVAS.dark.halo,
      ),
    ).toBe(true);
  });
  ...
  it('neighbour and hover rings use the plan radii', () => {
    ...
    expect(
      ctx.calls.some(
        (c) => c.m === 'arc' && c.args[2] === 13.4 && c.strokeStyle === CANVAS.dark.nbRing,
      ),
    ).toBe(true);
    expect(
      ctx.calls.some(
        (c) => c.m === 'arc' && c.args[2] === 14 && c.strokeStyle === CANVAS.dark.hoverRing,
      ),
    ).toBe(true);
  });
  ...
  it('a ghost (removed) node renders hollow + dimmed + the removed outline — NOT the fault treatment', () => {
    ...
    expect(
      ctx.calls.some((c) => c.m === 'stroke' && c.strokeStyle === hexa(CANVAS.dark.faultCss, 0.95)),
    ).toBe(false);
    expect(ctx.calls.some((c) => c.m === 'setLineDash' && c.args[0].join() === '2,3')).toBe(false);
    expect(ctx.calls.some((c) => c.m === 'moveTo' && c.args[0] === 95 && c.args[1] === 105)).toBe(
      false,
    );
    ...
    const outlineStroke = ctx.calls.find(
      (c) => c.m === 'stroke' && c.strokeStyle === OUTLINE_COLOR.removed,
    );
    ...
    const shadowSets = ctx.calls.filter(
      (c) => c.m === 'set' && c.prop === 'shadowBlur' && c.args[0] !== 0,
    );
  ...
  it('the ghost flag alone (no state) gets the same treatment', () => {
    ...
    expect(ctx.calls.some((c) => c.m === 'stroke' && c.strokeStyle === OUTLINE_COLOR.removed)).toBe(
      true,
    );
  });
  ...
  it('3D sorts painter-style by d descending and scales alpha by depth', () => {
    ...
    const nodeFills = ctx.calls.filter(
      (c) =>
        c.m === 'fill' &&
        c.args.length === 0 &&
        ctx.calls[ctx.calls.indexOf(c) - 1]?.m === 'arc' &&
        ctx.calls[ctx.calls.indexOf(c) - 1]?.args[2] >= 7,
    );
...
describe('drawHosts — host boxes', () => {
  ...
  it('draws host boxes BEFORE nodes, dashed [5,4], padded r+15 with y0-6', () => {
    ...
    const firstNodeFill = ctx.calls.findIndex(
      (c) =>
        c.m === 'fill' &&
        ctx.calls[ctx.calls.indexOf(c) - 1]?.m === 'arc' &&
        ctx.calls[ctx.calls.indexOf(c) - 1]?.args[2] === 8,
    );
    ...
    expect(
      ctx.calls.some((c) => c.m === 'lineTo' && c.args[0] === 163 - 12 && c.args[1] === 31),
    ).toBe(true);
  });
  ...
  it('uses the selected host tokens when the host is selected', () => {
    ...
    expect(
      ctx.calls.some((c) => c.m === 'stroke' && c.strokeStyle === CANVAS.dark.hostStrokeSel),
    ).toBe(true);
  });
});
...
describe('draw — non-finite resilience', () => {
  ...
  it('skips a link with a NaN endpoint instead of throwing — and still draws the healthy link', () => {
    ...
    expect(ctx.calls.some((c) => c.m === 'moveTo' && c.args[0] === 100 && c.args[1] === 100)).toBe(
      true,
    );
    expect(ctx.calls.some((c) => c.m === 'lineTo' && c.args[0] === 300 && c.args[1] === 100)).toBe(
      true,
    );
  });
...
describe('draw — incident urgency (I4)', () => {
  const urgency = (over = {}) =>
    new Map([
      [
        'hot',
  ...
  it('paints the saturated-red urgency treatment for a ranked incident', () => {
    const ctx = makeCtx();
    draw(
      ctx,
      baseModel({
        now: 0,
        nodes: [measuredNode({ id: 'hot', x: 400, y: 300 })],
        urgency: urgency(),
      }),
    );
    // a fill in the theme fault red — the urgency disc
    expect(
      ctx.calls.some((c) => c.m === 'fill' && c.fillStyle === hexa(CANVAS.dark.faultCss, 0.92)),
    ).toBe(true);
    // the dashed halo ring
    expect(ctx.calls.some((c) => c.m === 'setLineDash' && c.args[0][0] === 4)).toBe(true);
  });

  // radius of the arc immediately preceding a call matching `pred`
  const radiusBefore = (ctx, pred) => {
    const idx = ctx.calls.findIndex(pred);
    if (idx < 0) return undefined;
    for (let i = idx; i >= 0; i -= 1) if (ctx.calls[i].m === 'arc') return ctx.calls[i].args[2];
    return undefined;
  };

  it('pulses: the disc radius changes with the animation clock', () => {
    const radius = (now) => {
      const ctx = makeCtx();
      draw(
        ctx,
        baseModel({
          now,
          nodes: [measuredNode({ id: 'hot', r: 10, x: 400, y: 300 })],
          urgency: urgency(),
        }),
      );
      return radiusBefore(
        ctx,
        (c) => c.m === 'fill' && c.fillStyle === hexa(CANVAS.dark.faultCss, 0.92),
      );
    };
    expect(radius(0)).not.toBe(radius(400));
  });

  it('halo survives full zoom-out: a screen-space floor independent of camera zoom', () => {
    const ctx = makeCtx();
    draw(
      ctx,
      baseModel({
        now: 0,
        k: 0.0005, // fully-zoomed-out hairball: the node itself is sub-pixel
        nodes: [measuredNode({ id: 'hot', r: 10, x: 400, y: 300 })],
        urgency: urgency(),
      }),
    );
    const arcs = ctx.calls.filter((c) => c.m === 'arc' && c.args[0] === 400 && c.args[1] === 300);
    expect(Math.max(...arcs.map((c) => c.args[2]))).toBeGreaterThanOrEqual(14);
  });

  it('draws the urgency layer on top of every other node', () => {
    const ctx = makeCtx();
    draw(
      ctx,
      baseModel({
        now: 0,
        nodes: [
          measuredNode({ id: 'hot', x: 100, y: 100 }),
          measuredNode({ id: 'calm', kind: 'job', x: 400, y: 300 }),
        ],
        urgency: urgency(),
      }),
    );
    const redDisc = ctx.calls.findIndex(
      (c) => c.m === 'fill' && c.fillStyle === hexa(CANVAS.dark.faultCss, 0.92),
    );
    const calmFill = ctx.calls.findIndex(
      (c) => c.m === 'fill' && c.fillStyle === kindHue('job', 'dark'),
    );
    expect(redDisc).toBeGreaterThan(calmFill);
    // the white-hot core is the very last paint of the frame
    expect(ctx.calls[ctx.calls.length - 1].fillStyle).toBe(hexa('#FFFFFF', 0.9));
  });

  it('leaves non-urgent nodes untouched when an urgency map is present', () => {
    const ctx = makeCtx();
    draw(
      ctx,
      baseModel({
        now: 0,
        nodes: [measuredNode({ id: 'calm', x: 100, y: 100 })],
        urgency: urgency(),
      }),
    );
    expect(ctx.calls.some((c) => c.fillStyle === hexa(CANVAS.dark.faultCss, 0.92))).toBe(false);
  });

  it('never alarms a diff ghost with the urgency treatment', () => {
    const ctx = makeCtx();
    draw(
      ctx,
      baseModel({
        now: 0,
        nodes: [measuredNode({ id: 'hot', state: 'removed', ghost: true, x: 100, y: 100 })],
        urgency: urgency(),
      }),
    );
    expect(ctx.calls.some((c) => c.fillStyle === hexa(CANVAS.dark.faultCss, 0.92))).toBe(false);
