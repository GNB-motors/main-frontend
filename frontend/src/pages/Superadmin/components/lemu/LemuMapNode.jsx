...
const LemuMapNode = ({
  ...
  const tooltipText = useMemo(() => {
    if (kind === 'route' && extra) {
      return `n ${extra.n ?? 0} · err ${extra.err ?? 0} · p95 ${extra.p95 ?? '—'}ms`;
    }
    if (kind === 'model' && extra) {
      const sum = (extra.find || 0) + (extra.insert || 0) + (extra.update || 0) + (extra.del || 0) + (extra.agg || 0);
      return `ops ${sum} · find ${extra.find ?? 0} · insert ${extra.insert ?? 0} · update ${extra.update ?? 0}`;
    }
    if (kind === 'job' && extra) {
    ...
    >
      ...
      {state && state !== 'nothing' && (
        <span className="lemu-node__state">
          <LemuStatusChip state={state} label={state} />
        </span>
      )}
      {hasFinding && (
