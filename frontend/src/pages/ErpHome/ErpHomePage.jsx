...
const ErpHomePage = () => {
  ...
  const funnelData = [
    ...
    {
      name: 'Placed Today',
      count: pCounts.placementsToday || 0,
      color: '#f59e0b',
      route: '/erp/pipeline?tab=placement',
    },
    {
      name: 'Pending CN',
      count: pCounts.pendingCns || 0,
      color: '#8b5cf6',
      route: '/erp/pipeline?tab=trips',
    },
    {
      name: 'Pending Close',
      count: pCounts.pendingTripClose || 0,
      color: '#ec4899',
      route: '/erp/pipeline?tab=trips',
    },
    {
      name: 'Pending POD',
      count: pCounts.pendingPods || 0,
      color: '#06b6d4',
      route: '/erp/pipeline?tab=trips',
    },
    {
      name: 'Pending Unload',
      count: pCounts.pendingUnloadings || 0,
      color: '#10b981',
      route: '/erp/pipeline?tab=trips',
    },
    {
      name: 'Pending Bill',
      count: pCounts.pendingBillSubmissions || 0,
      color: '#00c896',
      route: '/erp/billing?tab=bills',
    },
  ];
      ...
      >
        ...
        >
          ...
          <span style={{ fontSize: '12px', color: '#64748b' }}>
            {pCounts.activeTrips?.PLACED || 0} Placed · {pCounts.activeTrips?.DISPATCHED || 0}{' '}
            Dispatched
          </span>
        </div>
        ...
        >
          ...
          <div
            style={{
              fontSize: '28px',
              fontWeight: 700,
              color: pCounts.pendingApprovals > 0 ? '#b45309' : '#1a202c',
            }}
          >
        ...
        >
          ...
          <span
            style={{
              fontSize: '12px',
              color: financials.receivablesOverdue > 0 ? '#dc2626' : '#64748b',
            }}
          >
            {financials.receivablesOverdue > 0
              ? `${formatMoney(financials.receivablesOverdue)} Overdue`
              : 'All current'}
          </span>
        </div>
        ...
        >
          ...
          <span style={{ fontSize: '12px', color: '#64748b' }}>
            Vendors: {formatMoney(financials.vendorPayables)} · Suppliers:{' '}
            {formatMoney(financials.supplierPayables)}
          </span>
        </div>
      </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              >
                ...
                <span style={{ fontSize: '14px', fontWeight: 700 }}>
                  {pCounts.podAgeing?.from7to14d || 0}
                </span>
              </div>
              <div
                style={{
                    ...
                    { header: 'Vehicle', render: (r) => r.vehicleNumber || '—' },
                    {
                      ...
                      render: (r) => (
                        <button
                          className="btn btn-primary"
                          style={{ padding: '6px 12px', fontSize: '12px' }}
                          onClick={() => openTripAction(r, 'close')}
                        >
                          Close Trip
                        </button>
                      ),
                ...
                : queueTab === 'pods'
                  ? [
                      ...
                      { header: 'Party', render: (r) => r.partyName || '—' },
                      {
                        ...
                        render: (r) => (
                          <StatusBadge
                            status={r.isOverdue ? 'DANGER' : 'NEUTRAL'}
                            label={`${r.ageingDays}d`}
                          />
                        ),
                      },
                      {
                  ...
                  : queueTab === 'unloading'
                    ? [
                        ...
                        {
                          header: 'Loaded Qty',
                          render: (r) => `${r.loadedQty} ${r.qtyUnit || 'KL'}`,
                        },
                        {
                          header: 'Action',
                          render: (r) => (
                            <button
                              className="btn btn-primary"
                              style={{ padding: '6px 12px', fontSize: '12px' }}
                              onClick={() => openTripAction(r, 'unloading')}
                            >
                              Enter Unloading
                            </button>
                          ),
                    ...
                    : [
                        {
                          header: 'Party',
                          render: (r) => <strong>{r.partyName || r.partyId?.name}</strong>,
                        },
                        { header: 'Unloadings', render: (r) => r.unloadings?.length || 1 },
                        {
                          header: 'Total Value',
                          render: (r) => formatMoney(r.totalAmount || r.netReceivable),
                        },
                        {
                          header: 'Action',
                          render: () => (
