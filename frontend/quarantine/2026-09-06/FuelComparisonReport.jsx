...
const SyncStatusBar = ({ status, isLoading, error, onRefresh }) => {
  ...
  if (error) {
    return (
      <div className="fuel-status-bar">
        <Alert severity="warning" className="py-1 text-[13px]">
          {error}
        </Alert>
      </div>
    );
        ...
        <tbody>
          {records.length === 0 ? (
            ...
            records.map((rec) => (
              ...
              >
                <td>
                  <div className="cell-primary">{rec.vehicleId?.registrationNumber || '—'}</div>
                </td>
                <td>
    ...
    <div className="p-6">
      ...
      {activeTab === 'all' && !isLoadingStatus && status?.flagged > 0 && (
        <div className="fuel-flagged-alert">
          ...
          <button className="fuel-flagged-alert-link" onClick={() => setActiveTab('flagged')}>
            View flagged →
          </button>
        </div>
      )}
